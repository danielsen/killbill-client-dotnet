...
namespace KillBillClient.Interfaces.Managers
{
    public interface IKillBillInvoiceManager
    {
        ...
        Task<List<InvoiceItem>> CreateExternalCharges(IEnumerable<InvoiceItem> externalCharges, DateTime? requestedDate,
        ...
        Task<Invoice> GetInvoice(int invoiceNumber, RequestOptions inputOptions, bool withItems = false,
            bool withChildrenItems = false, AuditLevel auditLevel = AuditLevel.NONE);

        Task<Invoice> GetInvoice(string invoiceIdOrNumber, RequestOptions inputOptions, bool withItems = false,
            bool withChildrenItems = false, AuditLevel auditLevel = AuditLevel.NONE);

        // INVOICES
        Task<Invoices> GetInvoices(RequestOptions inputOptions);

        Task<Invoices> GetInvoices(bool withItems, long offset, long limit, RequestOptions inputOptions,
            AuditLevel auditLevel = AuditLevel.NONE);

        Task<Invoices> GetInvoicesForAccount(Guid accountId, RequestOptions inputOptions, bool withItems = false,
            bool unpaidOnly = false, bool includeMigrationInvoices = false, AuditLevel auditLevel = AuditLevel.NONE);
