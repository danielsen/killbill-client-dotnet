...
namespace KillBillClient.Implementations.Managers
{
    public class KillBillInvoiceManager : KillBillBaseManager, IKillBillInvoiceManager
    {
        ...
        private async Task<IEnumerable<InvoiceItem>> CreateExternalCharges(Guid accountId,
        ...
        private async Task<Invoices> SearchInvoices(string key, long offset, long limit, AuditLevel auditLevel,
        ...
        {
            return await GetInvoiceByIdOrNumber(invoiceNumber.ToString(), inputOptions, withItems, withChildrenItems,
                auditLevel);
        }
        ...
        public async Task<Invoice> GetInvoice(string invoiceIdOrNumber, RequestOptions inputOptions,
            bool withItems = false, bool withChildrenItems = false, AuditLevel auditLevel = DefaultAuditLevel)
        {
            return await GetInvoiceByIdOrNumber(invoiceIdOrNumber, inputOptions, withItems, withChildrenItems,
                auditLevel);
        }
        ...
        {
            return await CreateExternalCharges(externalCharges, requestedDate, autoPay, autoCommit, null, null,
                inputOptions);
        }
            ...
            string transactionExternalKey, RequestOptions inputOptions)
        {
            ...
            {
                var invoiceItems = await CreateExternalCharges(accountId, externalChargesPerAccount[accountId],
                    requestedDate, autoPay, autoCommit, paymentExternalKey, paymentExternalKey, inputOptions);
